...
fn simulate_second(robots: &Vec<Robot>, second_num: i64, width: i64, height: i64) -> Vec<(i64, i64)> {
    let positions: Vec<(i64, i64)> =  robots.iter().map(|robot| {
...
fn print_locations(robots: &Vec<Robot>, second_num: i64, width: i64, height: i64) -> String {
    ...
    lines.extend(dot_matrix.iter().map(|line| {line.iter().collect::<String>()}));
...
fn part2_with_input(input: &str) -> i64 {
    let robots = parse_robots(input);
    let interesting_seconds: Vec<i64> = (20..100).map(|i| {
        vec![86+(i*101), 154+(i*103)]
    }).flatten().collect();
    let drawings = interesting_seconds.into_par_iter().map(|second_num| {
        print_locations(&robots, second_num, 101, 103)
    }).collect::<Vec<String>>();
